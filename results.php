...
$db->query('set @@session.time_zone = "-4:00"');
	if(isset($_POST['save'])){
		//die("conn made");
		echo("conn made");
		$auth = urlencode("email=api@instantcard.net&password=instantcardapitest");
		/*
		...
		$ch = curl_init();
    		curl_setopt($ch, CURLOPT_URL, 'https://staging.core.instantcard.net/api/v2/authenticate?email=api@instantcard.net&password=instantcardapitest');
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_HTTPHEADER, array(
